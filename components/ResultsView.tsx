...
import { 
  ...
  Target as TargetIcon, CheckCircle2, Bookmark, PlusCircle, Camera, Fingerprint, Eye
} from 'lucide-react';
...
const OpticalReconTerminal: React.FC<{ image: string, identification?: SearchState['identification'], isLoading: boolean }> = ({ image, identification, isLoading }) => {
  return (
    <div className="w-full bg-slate-900 rounded-[3rem] p-6 md:p-10 border border-slate-800 shadow-2xl relative overflow-hidden animate-in fade-in slide-in-from-top-4 duration-500">
      ...
      <div className="relative z-10 flex flex-col md:flex-row gap-8">
        {/* The Image Terminal */}
        <div className="relative w-full md:w-[300px] aspect-square rounded-[2rem] overflow-hidden border border-white/10 shadow-inner group">
          <img src={image} alt="Scouted Target" className="w-full h-full object-cover" />
          
          {/* Scanning Overlay */}
          <div className="absolute inset-0 pointer-events-none">
            <div className="absolute inset-x-0 h-0.5 bg-indigo-500/60 shadow-[0_0_15px_rgba(99,102,241,0.8)] top-0 animate-[scan_2s_ease-in-out_infinite]"></div>
            <div className="absolute inset-0 border-[20px] border-indigo-500/5"></div>
            <div className="absolute top-4 left-4 border-l-2 border-t-2 border-indigo-400 w-6 h-6"></div>
            <div className="absolute top-4 right-4 border-r-2 border-t-2 border-indigo-400 w-6 h-6"></div>
            <div className="absolute bottom-4 left-4 border-l-2 border-b-2 border-indigo-400 w-6 h-6"></div>
            <div className="absolute bottom-4 right-4 border-r-2 border-b-2 border-indigo-400 w-6 h-6"></div>
            
            <div className="absolute inset-0 flex items-center justify-center">
              <Crosshair className="w-12 h-12 text-indigo-400/30 animate-pulse" />
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 space-y-6">
          <div className="flex items-center gap-3">
             ...
             <div>
                <h3 className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.3em]">Optical Intel Analysis</h3>
                <div className="flex items-center gap-2">
                  <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded ${identification ? 'bg-emerald-500 text-white' : 'bg-amber-500 text-slate-900 animate-pulse'}`}>
                    {identification ? 'Resolved' : 'Reviewing Picture...'}
                  </span>
                </div>
             </div>
          </div>
            ...
            {identification ? (
              <div className="animate-in fade-in duration-700">
                <div className="bg-white/5 border border-white/10 rounded-2xl p-6 space-y-4">
                   <div>
                     <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest block mb-1">Target Identity</span>
                     <h4 className="text-2xl font-black text-white uppercase tracking-tight">{identification.name}</h4>
                   </div>
                   <div className="space-y-2">
                     <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest block">Visual Markers</span>
                     <p className="text-sm text-slate-300 font-medium leading-relaxed italic border-l-2 border-indigo-500 pl-4">
                       "{identification.analysis}"
                     </p>
                   </div>
                   <div className="flex items-center gap-2 pt-2">
                      <div className="text-[9px] font-black text-indigo-400 uppercase tracking-widest">Confidence Index</div>
                      <div className="flex-1 h-1.5 bg-white/5 rounded-full overflow-hidden">
                        <div className="h-full bg-indigo-500" style={{ width: `${identification.confidence}%` }}></div>
                      </div>
                      <span className="text-[10px] font-black text-white">{identification.confidence}%</span>
                   </div>
                </div>
                <div className="flex items-center gap-2 mt-4 text-[10px] font-black text-emerald-400 uppercase tracking-[0.2em] animate-pulse">
                   <TargetIcon className="w-4 h-4" /> Initiating Market Strike Mission...
                </div>
              </div>
            ) : (
              <div className="space-y-4 opacity-50">
                 ...
                 <div className="space-y-2">
                   <div className="h-3 w-2/3 bg-white/10 rounded-full animate-pulse"></div>
             ...
             <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-10">
                <div className="space-y-6">
                   <div className="space-y-3">
                      <h6 className="text-[9px] font-black text-slate-500 uppercase tracking-widest">ValuNinja Advice</h6>
...
const StrategicComparisonMatrix: React.FC<{ 
  products: Product[], 
  currencySymbol: string,
  onSaveToVault: (product: Product) => void,
  isTracked: (id: string) => boolean
}> = ({ products, currencySymbol, onSaveToVault, isTracked }) => {
  const categories = [
            ...
            <tr className="bg-slate-50 border-b border-slate-100">
              ...
              {products.map(p => (
                <th key={p.id} className="p-6 text-center min-w-[200px]">
                  ...
                  <div className="text-sm font-black text-slate-900 line-clamp-1 uppercase mb-3">{p.name}</div>
                  <button 
                    onClick={() => onSaveToVault(p)}
                    disabled={isTracked(p.id)}
                    className={`mx-auto flex items-center gap-2 px-3 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest transition-all active:scale-95 border ${isTracked(p.id) ? 'bg-emerald-50 text-emerald-600 border-emerald-200 cursor-default shadow-none' : 'bg-white text-indigo-600 border-indigo-100 hover:border-indigo-300 hover:bg-indigo-50 shadow-sm'}`}
                  >
                    {isTracked(p.id) ? <><Check className="w-3 h-3" /> In My List</> : <><BookmarkPlus className="w-3 h-3" /> Add to My List</>}
                  </button>
                </th>
              ))}
            </tr>
                    ...
                    <div className="space-y-4">
                       <div className="flex items-center justify-center gap-3 mb-2">
                         ...
                         <button 
                            onClick={() => onSaveToVault(p)}
                            disabled={isTracked(p.id)}
                            className={`p-2.5 rounded-xl transition-all active:scale-95 disabled:opacity-40 border ${isTracked(p.id) ? 'bg-emerald-500/20 border-emerald-500/40 text-emerald-400' : 'bg-white/10 border-white/10 hover:bg-indigo-600 text-white'}`}
                            title="Save to Arsenal"
                         >
                           {isTracked(p.id) ? <Check className="w-4 h-4" /> : <BookmarkPlus className="w-4 h-4" />}
                         </button>
                       </div>
        ...
        <div className="flex-grow max-w-5xl py-10 space-y-12">
          ...
          {hasResults && (
            <div className={`space-y-16 animate-in fade-in duration-1000 ${isTransitioning ? 'opacity-40 pointer-events-none' : 'opacity-100'}`}>
                  ...
                  <div className="relative z-10 p-10 md:p-14 flex flex-col lg:flex-row gap-12 items-center">
                    ...
                    <div className="flex flex-col items-center md:items-end gap-6 md:gap-8 min-w-[300px] md:min-w-[340px]">
                      <div className="flex flex-col items-center md:items-end leading-none">
                        ...
                        <div className="text-[8px] md:text-[10px] font-black text-slate-500 uppercase tracking-widest">{topProduct.storeName || 'Verified Hub'}</div>
                      </div>
              ...
              <StrategicComparisonMatrix 
                products={sortedProducts} 
                currencySymbol={getDisplayCurrency(sortedProducts[0]?.currency || '$')} 
                onSaveToVault={onSaveToVault}
                isTracked={isTracked}
              />

              {/* Mission Recon Analysis */}
              <MissionControlCenter 
              ...
              <div className="bg-white rounded-[3.5rem] shadow-xl border-2 border-slate-100 p-10 md:p-14">
                  ...
                  <AttributeForm 
                    ...
                    onUpdateResultsLimit={onResultsLimitUpdate}
